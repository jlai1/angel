...
$(document).keydown (e) ->
  if (e.keyCode == 16) 
    $('.svg-element').tooltip("show")

$(document).keyup (e) ->
  if (e.keyCode == 16) 
    $('.svg-element').tooltip("hide")
    # alert(e.which + " or Shift was pressed")


# transition

Template.layout.events 'click #50_day_moving_average_button': (event) ->
  d3.selectAll("circle").transition()
    .attr("transform", (d) ->
      "translate(0, " + 1000*(d.price - d.day50MovingAvg)/d.price + ")"
      )
    # .attr("transform", "translate(0, 300)")
    .duration(5000)

Template.layout.events 'click #200_day_moving_average_button': (event) ->
  d3.selectAll("circle").transition()
    .attr("transform", (d) ->
      "translate(0, " + 1000*(d.price - d.day200MovingAvg)/d.price + ")"
      )
    # .attr("transform", "translate(0, 300)")
    .duration(5000)


Template.layout.events 'click #today_button': (event) ->
  d3.selectAll("circle").transition()
    .attr("transform", (d) ->
      # "translate(0, " + d.roe + 200 + ")"
      "translate(0, " + d.roe + 200 + ")"
      )
    # .attr("transform", "translate(0, 300)")
    .duration(5000)
...
#
  ...
  # svgContainer = d3.select('svg').attr('width', 500).attr('height', 0)
  ...
  jsonCircles = [
    ...
    #   'color': 'red'
    #   'data': 0
    #   'company_name': 'placeholder3'
    #   'id': 'placeholder3'
    # }
  ]

  # loop sp500
    # call Session.get('FB') to get price
    # to plot

  quotes_sp500.forEach (quote) ->
    if (Session.get(quote.symbol) != undefined)
      roe = parseFloat Session.get(quote.symbol).returnOnEquity
      dividendYield = parseFloat Session.get(quote.symbol).dividendYield
      color = "black"
      if (Session.get(quote.symbol).profitMargin > 0)
        color = "green"
      else
        color = "red"
      jsonCircles.push({
        'x_axis': dividendYield * 100 + 50
        'y_axis': roe + 200
        'radius': Math.abs Session.get(quote.symbol).profitMargin/2
        'color': color
        'company_name': Session.get(quote.symbol).companyName
        'id': Session.get(quote.symbol).symbol
        'profit_margin': Session.get(quote.symbol).profitMargin
        'day50MovingAvg': Session.get(quote.symbol).day50MovingAvg
        'day200MovingAvg': Session.get(quote.symbol).day200MovingAvg
        'price': Session.get(quote.symbol+"_price")
        'roe': roe
      })

  $("#hover-text2").attr("data-toggle", "tooltip").attr("title", "Hooray2")
  ...
  circles = svgContainer.selectAll('circle').data(jsonCircles).enter().append('circle').attr("class", "svg-element")
    .attr('cx', (d) ->
        d.x_axis
      )
    .attr('cy', (d) ->
    ...
    # .attr('day5ChangePercent', (d) ->
    #     d.day5ChangePercent
    #   )
    # .attr('day30ChangePercent', (d) ->
    #     d.day30ChangePercent
    #   )
    # .attr('day50ChangePercent', (d) ->
    #     d.day50ChangePercent
    #   )
    # .attr('day200ChangePercent', (d) ->
    #     d.day200ChangePercent
    #   ) 
    .style("opacity", 0.5)
    .call(d3.drag()
...
# generated by js2coffee 2.2.0
  ...
  # $('.svg-element').tooltip({title:'some title', container:'body'}) 

  # $('[data-toggle="tooltip"]').tooltip()


  # circleAttributes = circles.attr('cx', (d) ->
  #   d.x_axis
  # ).attr('cy', (d) ->
  #   d.y_axis
  # ).attr('r', (d) ->
  ...
  # ).attr('id', (d) ->
  #   d.id
  # ).attr('profit_margin', (d) ->
  #   d.profit_margin
  # ).style("opacity", 0.5)

  # $('.svg-element').tooltip({title: this, container:'body'}) 
  # $('.svg-element').tooltip({title: 'some title', container:'body'}) 
  
  i = 0
  while i < $('.svg-element').length
    # console.log d3.selectAll("circle")[0][i].getAttribute("company_name")
    # company_name = d3.selectAll("circle")[0][i].getAttribute("company_name")
    # id = d3.selectAll("circle")[0][i].getAttribute("id")
    company_name = $(".svg-element")[i].getAttribute("company_name")
    id = $(".svg-element")[i].getAttribute("id")
    $("#"+id).tooltip({title: company_name, container:'body'}) 
    i++


  # for elem of $('.svg-element')
  #   console.log elem


  # $('.svg-element').on('mouseover', ->
  #   console.log this.getAttribute("company_name")
  #   )
  # circles.tooltip({title:'some title', container:'body'}) 
